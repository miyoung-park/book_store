        ...
        <div class="bookImage_section">
          ...
          <div class="bookImage" >
            ...
            <div v-for="image in bookImages" :key="image.fileSeq">
              <img class="previewImage" :src="concatPath(image)"> <!-- 기존 이미지 로드 -->
  ...
  methods: {
    ...
    showImage(bookNewImage){
      if( bookNewImage.length > 0 ) {
       const fileNum = bookNewImage.length + this.bookImages.length
        if( bookNewImage.length > 4 || fileNum > 4) {
          alert('이미지는 총 4개까지만 업로드 가능합니다.');

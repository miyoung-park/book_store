...
export default {
  ...
  inject:['bookService' , 'rentalService' , 'userService'],
  created() {
  ...
  methods: {
    ...
    async rentalBook(){
      ...
      const userInfo = await this.userService.getUserDetailById();
      this.totalPoint = userInfo.userPoint;
      ...
      if( dayCount != null) {
        ...
        returnDate.setDate( returnDate.getDate() + Number( dayCount )  + 1);
        const formatDate = this.dateFormat.formatDate( returnDate );
        this.rentalInfo.predictReturnDt = formatDate;
